...
## 更新摘要
**所做更改**
- 新增依赖管理维护指南，重点涵盖package-lock.json重新生成和npm ci问题解决方案
- 更新了 GitHub Actions 工作流配置，将 Node.js 版本升级至 22，添加 JavaScript 动作兼容性环境变量
- 标准化了 Python 测试命令，改进了 CI/CD 管道配置
- 更新了开发环境搭建指南，反映最新的 Node.js 和 Python 版本要求
- 完善了 CI/CD 流程说明，包括前端测试和构建流程
...
J["Axios 封装<br/>src/api/index.ts"]
...
- [frontend/src/stores/chatStore.ts:4-417](file://frontend/src/stores/chatStore.ts#L4-L417)
...
### package-lock.json 重新生成与维护

#### 何时需要重新生成 package-lock.json
- 当 package.json 中的依赖版本发生重大变更时
- 当遇到 npm ci 安装失败或依赖冲突时
- 当团队成员使用不同 Node.js 版本导致安装不一致时
- 当 CI/CD 流程中出现依赖安装问题时
...
5. **提交更新的 package-lock.json**
   ```bash
   git add package-lock.json
   git commit -m "Update package-lock.json for dependency consistency"
   ```
...
1. **Node.js 版本不匹配**
   - 确保使用 Node.js 22+ 版本
   - 检查 .nvmrc 或 GitHub Actions 中的 Node.js 版本配置
...
**GitHub Actions 工作流中的最佳实践：**
- 使用 `npm ci` 而非 `npm install` 以确保依赖一致性
- 启用 npm 缓存以提高构建速度
- 在安装前清理缓存
...
- [frontend/package-lock.json:1-800](file://frontend/package-lock.json#L1-L800)
- [.github/workflows/ci.yml:49-51](file://.github/workflows/ci.yml#L49-L51)

## 结论
本指南提供了 PaperChat 项目的开发环境、代码规范、开发流程、测试策略、性能优化与故障排除的系统性说明。新增的依赖管理维护指南特别强调了 package-lock.json 的重要性和 npm ci 的最佳实践，这对于确保团队协作的一致性和 CI/CD 流程的稳定性至关重要。建议团队在开发中严格遵循配置与规范，利用现有 WebSocket 与 RAG 能力，持续优化前端构建与后端并发处理，保障系统的稳定性与可维护性。
...
**更新** 根据最新的 CI/CD 配置，前端开发环境现在要求 Node.js 22+ 版本，以确保与 GitHub Actions 工作流的一致性。
...
- 持续集成：GitHub Actions 工作流包含完整的 CI/CD 管道，包括后端测试、前端测试、代码质量检查和构建

**更新** CI/CD 管道现已标准化，后端使用 Python 3.11，前端使用 Node.js 22，测试命令已标准化为 python -m pytest 和 npm test。

**章节来源**
- [.github/workflows/ci.yml:1-64](file://.github/workflows/ci.yml#L1-L64)

### 测试策略与质量保证
- 单元测试：针对服务层（如 llm_service、search_service）编写异步测试，模拟外部依赖
- 集成测试：使用 FastAPI TestClient 测试路由与中间件行为，覆盖 WebSocket 路由的关键分支
- 端到端测试：使用 Vitest 进行前端组件测试，支持 jsdom 环境和国际化配置
- 质量门禁：ESLint 代码质量检查，Vite 构建产物大小告警，覆盖率门槛（建议引入）

**更新** 测试配置已标准化，后端测试使用 pytest，前端测试使用 vitest，测试命令分别为 python -m pytest tests/ -v 和 npm test。
...
- CI/CD 验证：确保本地测试通过，符合 GitHub Actions 工作流要求

**更新** 贡献流程现已包含 CI/CD 验证要求，确保代码变更符合标准化的测试和构建流程。

**章节来源**
- [.github/workflows/ci.yml:1-64](file://.github/workflows/ci.yml#L1-L64)

### GitHub Actions 工作流配置
项目使用 GitHub Actions 进行持续集成和部署，包含以下关键配置：

- 环境变量：FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true
- 触发条件：在推送到 main/master 分支或创建拉取请求时触发
- 后端测试作业：使用 Python 3.11，运行 pytest 测试套件
- 前端测试作业：使用 Node.js 22，执行 ESLint 代码检查、单元测试和构建

**更新** 工作流配置已升级至 Node.js 22，添加了 JavaScript 动作兼容性环境变量，标准化了测试命令。

**章节来源**
- [.github/workflows/ci.yml:1-64](file://.github/workflows/ci.yml#L1-L64)

### Docker Compose 配置
项目提供完整的 Docker Compose 配置，支持一键启动完整的开发环境：

- 后端服务：构建后端镜像，映射 8000:8000 端口，挂载上传目录和数据库文件
- 前端服务：构建前端镜像，映射 80:80 端口，依赖后端服务
- 健康检查：自动监控后端服务健康状态

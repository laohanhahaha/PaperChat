...
## 更新摘要
**所做更改**
- 新增 PyTest 后端测试框架配置与基础设施
- 新增 Vitest 前端测试配置与测试环境设置
- 完善测试基础设施文档，包括全局 fixtures 和测试配置
- 更新测试策略以反映新的测试框架和配置
...
subgraph "前端"
K["App.jsx<br/>阅读与分析主流程"]
L["useWebSocket.js<br/>WebSocket Hook"]
M["chatStore.js<br/>Zustand 状态"]
end
subgraph "后端"
N["main.py<br/>应用入口与健康检查"]
O["routers/ws.py<br/>WebSocket 路由"]
P["handlers/analyze_handler.py<br/>分析处理器"]
Q["services/pdf_service.py<br/>PDF 服务"]
R["models/paper.py<br/>论文模型"]
S["database.py<br/>异步数据库"]
end
A --> B
B --> C
B --> D
...
- **测试依赖**
  - PyTest、pytest-asyncio、Vitest、React Testing Library、Jest DOM 等。
- **关键耦合点**
...
- **测试清单**
  ...
  - 用户体验：可访问性、性能感知、用户反馈
- **测试基础设施**
  - 后端：PyTest 配置、全局 fixtures、内存数据库

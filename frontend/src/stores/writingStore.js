...
const useWritingStore = create((set, get) => ({
  ...
  generateDraft: async (outlineSection, context = '', style = 'academic', onChunk = null) => {
    ...
    try {
      const response = await fetch('/api/writing/draft', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
  ...
  polishText: async (text, polishType = 'academic', onChunk = null) => {
    ...
    try {
      const response = await fetch('/api/writing/polish', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
  ...
  generateCitations: async (paperIds, format = 'apa') => {
    ...
    try {
      const response = await api.post('/writing/citations', {
        paper_ids: paperIds,
        format,
      });
  ...
  getFormats: async () => {
    try {
      const response = await api.get('/writing/formats');
